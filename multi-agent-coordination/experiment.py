...
class Agent:
    ...
    def sample_action_message(self, grid_state, messages, method="cat++"):
        ...
        if method == "cat++":
            action, action_uncertainty = StochasticCategorical.apply(action_probs.unsqueeze(0), None)
            message, message_uncertainty = StochasticCategorical.apply(message_probs.unsqueeze(0), None)
    ...
    def run_episode_direct(self, max_steps=20, method="cat++", verbose=False):
        ...
        for step in range(max_steps):
            ...
            for agent in self.agents:
                ...
                if method == "cat++":
                    action, _ = StochasticCategorical.apply(action_probs.unsqueeze(0), None)
                    message, _ = StochasticCategorical.apply(message_probs.unsqueeze(0), None)
                    action_idx = action.item()
                    message_idx = message.item()
                elif method == "categorical":
                    action = Categorical.apply(action_probs.unsqueeze(0))
    ...
    for method in all_results.keys():
        ...
        for run in range(n_runs):
            ...
            if method == "reinforce":
                ...
                rewards_history = optimize_multi_agent_reinforce(
                    system,
                ...
                rewards_history = optimize_multi_agent_direct(
                    system,
                    n_episodes=n_episodes,
                    lr=lr,
                    method=method
        ...
        all_results[method] = {"avg_final_reward": avg_reward, "avg_time_sec": avg_time}
...
if __name__ == "__main__":
    ...
    if total_reward.item() > 0:
        ...
        for method in methods:
            ...
            learning_curves[method] = rewards_history
