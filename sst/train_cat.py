...
from utils_cat import get_experiments_folder, get_experiment_name
from utils_cat import load_data, encode_onehot
from utils_cat import maybe_make_logits_symmetric
from utils_cat import nll_gaussian, kl_categorical_uniform, kl_gumbel
from utils_cat import sampling_edge_metrics
from utils_cat import sample_indep_edges
...
# Note: our custom estimator works for the indep case.
...
# Change default here: with custom estimator we want KL computed wrt probabilities, not Gumbel noise.
parser.add_argument("--use_gumbels_for_kl", type=eval, default=False,
                    choices=[True, False],
...
if args.mode == "train":
    ...
    if not os.path.exists("experiments_cat"):
        os.makedirs("experiments_cat")
    if not os.path.exists(os.path.join("experiments_cat", experiments_folder)):
        os.makedirs(os.path.join("experiments_cat", experiments_folder))
    experiment_name = get_experiment_name(args)
    experiment_folder = os.path.join(
        "experiments_cat", experiments_folder, experiment_name)
        ...
        for batch_idx, (data, relations) in enumerate(train_loader):
            ...
            if args.use_nvil or args.use_reinforce:
                ...
                if args.use_gumbels_for_kl:
                    logprob = (-(edge_weights - logits.unsqueeze(0)) - 
                ...
                if args.use_nvil:
                    ...
                    baseline_loss = (((loss_nll - loss_nll_ema).detach() - nvil_baseline) ** 2)
                    nvil_loss = (loss_nll +
                    ...
                    if args.reinforce_baseline == "ema":
                        ...
                        baseline = loss_nll.mean(0).unsqueeze(0)
...
def test():
    ...
    idx = 0
    for exp in os.listdir(os.path.join("experiments_cat", args.experiments_folder)):
        trial_path = os.path.join("experiments_cat", args.experiments_folder, exp)
...
if args.mode == "train":
    ...
    data_path = os.path.join(
        "experiments_cat", args.experiments_folder, "data_for_bootstrapping.pkl")
